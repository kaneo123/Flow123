...
// Centralized loyalty logic and Oliver's API integration
...
// Normalizes varied customer payloads into a consistent list
function parseCustomers(rawResponse: any): any[] {
  ...
  // Common shapes observed from Oliver's API
  ...
  // Fallback: single object treated as one customer
  if (candidates.length === 0 && rawResponse && typeof rawResponse === "object") {
    candidates.push(rawResponse);
  }

  console.log(`📊 Found ${candidates.length} customer candidate(s)`);
...
serve(async (req) => {
  ...
  console.log("📥 Incoming request", { method: req.method, url: req.url, search: url.search });
  ...
  if (req.method === "POST" || req.method === "PUT") {
    try {
      ...
      return jsonResponse({ error: "Invalid JSON body", detail: String(e) }, 400);
    }
  }
  ...
  if (!ADMIN_TOKEN) {
    ...
    return jsonResponse({ error: "Server configuration error", detail: "LOYALTY_ADMIN_TOKEN not configured" }, 500);
  }
  ...
  try {
    switch (action) {
      case "customer": {
        ...
        console.log("📥 Customer lookup response:", { status: upstream.status, body: bodyText.substring(0, 400) });
        ...
        try {
          ...
          return jsonResponse({ error: "Invalid response from loyalty provider", detail: bodyText.substring(0, 400) }, 502);
        }

        if (!upstream.ok) {
          return jsonResponse({ error: "Failed to find customer", detail: rawData }, upstream.status);
        }

        const customers = parseCustomers(rawData);
        console.log(`✅ Parsed ${customers.length} customer(s)`);
      ...
      case "rewards": {
        ...
        const [offersRes, couponsRes] = await Promise.all([
          fetch(offersUrl, { headers: { authorization: `Bearer ${ADMIN_TOKEN}`, role: "admin" } }),
          fetch(couponsUrl, { headers: { authorization: `Bearer ${ADMIN_TOKEN}`, role: "admin" } }),
        ]);

        const offersText = await offersRes.text();
        const couponsText = await couponsRes.text();

        console.log("📥 Rewards responses:", { offersStatus: offersRes.status, couponsStatus: couponsRes.status });
        ...
        if (!offersRes.ok || !couponsRes.ok) {
          return jsonResponse({
            ...
            upstreamStatus: { offers: offersRes.status, coupons: couponsRes.status },
          }, 502);
        }
      ...
      case "complete_payment": {
        ...
        const { orderId, userId, restaurantId, totalAmount, pointsPerPound = 1.0, reward } = requestBody;
        ...
        const pointsToAward = Math.floor(Number(totalAmount) * Number(pointsPerPound));
        ...
        const results: any = { orderId, pointsAwarded: false, rewardRecorded: false, errors: [] as any[] };
        ...
        if (pointsToAward > 0) {
          try {
            const pointsPayload = {
              ...
              orderDetails: `FlowTill POS | Order #${orderId.substring(0, 8)} | Bill £${Number(totalAmount).toFixed(2)}`,
            };
            ...
            console.log("📥 Points award response:", { status: pointsRes.status, body: pointsText.substring(0, 400) });
            ...
            if (pointsRes.ok) {
              ...
              results.errors.push({ step: "points_award", status: pointsRes.status, detail: pointsText });
              console.error("❌ Failed to award points:", pointsRes.status);
            }
          } catch (e) {
            results.errors.push({ step: "points_award", error: String(e) });
        ...
        if (reward?.id && reward?.type) {
          try {
            if (reward.type === "offer") {
              const offerPayload = { userId, offerId: reward.id, status: "redeemed", orderId };
              ...
              console.log("📥 Offer history response:", { status: offerRes.status, body: offerText.substring(0, 400) });

              if (offerRes.ok) {
                results.rewardRecorded = true;
                console.log("✅ Offer redemption recorded");
              } else {
                results.errors.push({ step: "offer_history", status: offerRes.status, detail: offerText });
              }
            } else if (reward.type === "coupon") {
              const couponPayload = { userId, couponId: reward.id, status: "redeemed", orderId };
              ...
              console.log("📥 Coupon history response:", { status: couponRes.status, body: couponText.substring(0, 400) });
              ...
              console.log("📥 Scratch coupon response:", { status: scratchRes.status, body: scratchText.substring(0, 400) });
            ...
            results.errors.push({ step: "reward_recording", error: String(e) });
            console.error("❌ Exception recording reward:", e);
          }
        }

        console.log("🏁 Payment completion results:", results);
        return jsonResponse(results, results.errors.length === 0 ? 200 : 207);
      }
